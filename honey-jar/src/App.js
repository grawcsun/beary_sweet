    ...
    <div className="min-h-screen relative overflow-hidden" style={{
      ...
      <div className="max-w-6xl mx-auto relative z-10 p-6">
        <header className="text-center py-6 mb-6">
          <h1 className="text-7xl font-bold mb-2 relative inline-block" style={{
            fontFamily: 'Georgia, serif',
            color: '#FFD700',
            textShadow: `
              3px 3px 0px #FFA500,
              6px 6px 0px #FF8C00,
              9px 9px 12px rgba(139, 69, 19, 0.5),
...
function AnimatedJar({ count, size = 'medium', onClick, isToday }) {
  ...
  useEffect(() => {
    ...
    if (isAdding) {
      // Adding honey drop - animate through frames
      ...
      // Removing honey drop - just jump to previous state without animation
      previousCount.current = count;
      return;
    }

    // Animate through frames for adding honey
    if (startFrame && endFrame) {
      setIsAnimating(true);
      const frames = Array.from({length: endFrame - startFrame + 1}, (_, i) => startFrame + i);
  ...
  const sizes = {
    small: { width: 100, height: 138 },
    medium: { width: 120, height: 160 },
    large: { width: 180, height: 240 },
    xlarge: { width: 240, height: 320 }
  };
...
function CozyBear({ name, flip, size = 'default' }) {
  const bearSize = size === 'large' ? { width: '240px', height: '240px', fontSize: '22px' } : { width: '180px', height: '180px', fontSize: '18px' };
      ...
      <div className="mb-6 p-6 rounded-3xl" style={{
        ...
        <div className="flex justify-between items-end mb-2">
          {getCurrentWeekJars().map((jar, idx) => (
            <div key={idx} className="flex flex-col items-center" style={{ marginBottom: '32px' }}>
              <AnimatedJar
                count={jar.count}
                size="small"
      ...
      <div className="mb-6 p-6 rounded-3xl relative" style={{
        ...
        <div className="p-4 rounded-2xl relative" style={{
          ...
          <div className="flex items-start gap-3">
            <span className="text-2xl">{bearEmoji}</span>
            <div>
    ...
    <div className="fixed inset-0 flex items-center justify-center p-4 z-50" style={{
      ...
      <div className="max-w-2xl w-full max-h-[90vh] overflow-y-auto p-8 rounded-3xl relative" style={{
        ...
        <button
          ...
          className="absolute left-4 top-1/2 -translate-y-1/2 p-3 rounded-full hover:bg-amber-100 transition-all"
          style={{ color: '#8B4513', background: 'rgba(255, 248, 220, 0.8)' }}
        >
          <ChevronLeft className="w-6 h-6" />
        </button>
        <button
          onClick={() => navigateDay(1)}
          className="absolute right-4 top-1/2 -translate-y-1/2 p-3 rounded-full hover:bg-amber-100 transition-all"
          style={{ color: '#8B4513', background: 'rgba(255, 248, 220, 0.8)' }}
        >
          <ChevronRight className="w-6 h-6" />
        </button>
